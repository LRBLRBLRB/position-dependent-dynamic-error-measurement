...
本目录是项目中的 IDS-3010 位移测量数据处理模块，用于处理由 TDMS 数据转换得到的 IDS 位移测量结果。当前主流程从位移序列出发，依次完成去噪、降采样、几何定位误差补偿、速度与加速度计算、运动分段、特征指标提取和结果绘图。

项目整体说明见 [../../../README.md](../../../README.md)。本文档只描述 `ids-3010` 子目录内的处理流程、入口脚本、输入输出和文件职责。
...
该脚本目前是面向单个实验文件的交互式研究脚本。运行前应先完成下方“运行前准备”中的路径、测点和阈值检查。
